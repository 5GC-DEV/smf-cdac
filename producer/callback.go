...
func BuildPfcpParam(smContext *smfContext.SMContext) *pfcpParam {
	...
	for i, pdr := range pfcpParam.pdrList {
		logger.PfcpLog.Infof("PDR[%d]: %+v", i, pdr)
	}
	for i, far := range pfcpParam.farList {
		logger.PfcpLog.Infof("FAR[%d]: %+v", i, far)
	}
	for i, qer := range pfcpParam.qerList {
		logger.PfcpLog.Infof("QER[%d]: %+v", i, qer)
	}

	/*for i, pdr := range pfcpParam.removePDR {
		logger.PfcpLog.Infof("Remove PDR[%d]: %+v", i, pdr)
	}
	for i, far := range pfcpParam.removeFAR {
		logger.PfcpLog.Infof("Remove FAR[%d]: %+v", i, far)
	}
	for i, qer := range pfcpParam.removeQER {
		logger.PfcpLog.Infof("Remove QER[%d]: %+v", i, qer)
	}*/
