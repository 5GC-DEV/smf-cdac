...
func SendPfcpSessionModificationRequest(
	...
	logger.PfcpLog.Debugf(
		"[SendPfcpSessionModificationRequest][Enter] Supi=%s PduSessionID=%d NodeID=%s SeqNum=%d",
		ctx.Supi,
		ctx.PDUSessionID,
		upNodeIDStr,
		seqNum,
	...
	if !ok {
		logger.PfcpLog.Errorf(
			"[SendPfcpSessionModificationRequest] PFCP Context not found for NodeID[%s]",
			upNodeIDStr,
		)
	...
	logger.PfcpLog.Debugf(
		"[SendPfcpSessionModificationRequest] LocalSEID=%d RemoteSEID=%d",
		pfcpContext.LocalSEID,
		pfcpContext.RemoteSEID,
	)

	logger.PfcpLog.Debugf(
		"[SendPfcpSessionModificationRequest] AddPDRs=%d AddFARs=%d AddQERs=%d RemovePDRs=%d RemoveFARs=%d RemoveQERs=%d",
		len(pdrList),
		len(farList),
		len(qerList),
		len(removePDR),
	...
	for i, pdr := range pdrList {
		logger.PfcpLog.Debugf(
			"[SendPfcpSessionModificationRequest] AddPDR[%d] PDRID=%d FARID=%d Precedence=%d",
			i,
			pdr.PDRID,
			pdr.FAR.FARID,
			pdr.Precedence,
	...
	for i, far := range farList {
		logger.PfcpLog.Debugf(
			"[SendPfcpSessionModificationRequest] AddFAR[%d] FARID=%d",
			i,
			far.FARID,
		)
	}
	...
	for i, qer := range qerList {
		logger.PfcpLog.Debugf(
			"[SendPfcpSessionModificationRequest] AddQER[%d] QERID=%d QFI=%d",
			i,
			qer.QERID,
			qer.QFI.QFI,
		)
	...
	for i, pdr := range removePDR {
		logger.PfcpLog.Debugf(
			"[SendPfcpSessionModificationRequest] RemovePDR[%d] PDRID=%d",
			i,
			pdr.PDRID,
		)
	}
	...
	for i, far := range removeFAR {
		logger.PfcpLog.Debugf(
			"[SendPfcpSessionModificationRequest] RemoveFAR[%d] FARID=%d",
			i,
			far.FARID,
		)
	}
	...
	for i, qer := range removeQER {
		logger.PfcpLog.Debugf(
			"[SendPfcpSessionModificationRequest] RemoveQER[%d] QERID=%d",
			i,
			qer.QERID,
		)
	}
	...
	if err != nil {
		logger.PfcpLog.Errorf(
			"[SendPfcpSessionModificationRequest] Build PFCP Session Modification Request failed: %v",
			err,
		)
	...
	logger.PfcpLog.Debugf(
		"[SendPfcpSessionModificationRequest] PFCP Message built successfully SeqNum=%d",
		seqNum,
	)
	...
	logger.PfcpLog.Debugf(
		"[SendPfcpSessionModificationRequest] Destination UPF Address=%s",
		upaddr.String(),
	)

	if factory.SmfConfig.Configuration.EnableUpfAdapter {

		logger.PfcpLog.Debugln(
			"[SendPfcpSessionModificationRequest] Sending PFCP Session Modification Request via UPF Adapter",
		)
		...
		if rsp, err := SendPfcpMsgToAdapter(
			...
			logger.PfcpLog.Debugf(
				"[SendPfcpSessionModificationRequest] Adapter Response Status=%d",
				rsp.StatusCode,
			)
			...
			if rsp.StatusCode == http.StatusOK {
				...
				logger.PfcpLog.Debugf(
					"[SendPfcpSessionModificationRequest] Adapter PFCP Response=%s",
					pfcpMsgString,
				)
				...
				if err != nil {
					logger.PfcpLog.Errorf(
						"parse pfcp session modify response failed: %v",
						err,
					)
				...
				logger.PfcpLog.Debugf(
					"[SendPfcpSessionModificationRequest] Handling Adapter PFCP Response LSEID=%d",
					eventData.LSEID,
				)
				...
				if err != nil {
					logger.PfcpLog.Errorf(
						"handle adapter pfcp response failed: %v",
						err,
					)
				}
			}
		...
		logger.PfcpLog.Debugf(
			"[SendPfcpSessionModificationRequest] Inserting PFCP Transaction SeqNum=%d",
			pfcpMsg.Sequence(),
		)
		...
		logger.PfcpLog.Debugf(
			"[SendPfcpSessionModificationRequest] Sending PFCP Session Modification Request to UPF LSEID=%d RemoteAddr=%s",
			eventData.LSEID,
			upaddr.String(),
		)
		...
		if err != nil {
			logger.PfcpLog.Errorf(
				"send pfcp session modify msg to upf error [%v]",
				err.Error(),
			)
	...
	logger.PfcpLog.Debugf(
		"[SendPfcpSessionModificationRequest][Exit] Supi=%s PduSessionID=%d SeqNum=%d",
		ctx.Supi,
		ctx.PDUSessionID,
		seqNum,
	)
